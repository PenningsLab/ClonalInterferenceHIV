...
source("Rscripts/ResistanceMutationsHIVDB.r")

text <- readLines("OriginalData/sequence.gb")
...
for (i in 1:length(PImuts$pos)){
	names(patientinfo)[min(which(substr(names(patientinfo),1,1)=="V"))]<-
	paste("PI",PImuts$pos[i],sep="")}
...
#PImuts
PIcolumns<-which(substr(names(patientinfo),1,2)=="PI")
for (p in 1:length(PImuts$pos)){
	patAA<-substr(AAseqs$AA[i],PImuts$pos[p],PImuts$pos[p])
	if (nchar(patAA)>0){
		if(grepl(patAA,PImuts$mut[p])){
			patientinfo[i,PIcolumns[p]]<-1
		}}}
}
...
for (p in c(NNRTIcolumns,NRTIcolumns,PIcolumns)){
	for (i in 1:length(patientoverview[,1])){
...
write.csv(patientoverview,"OriginalData/PatientOverview.csv")#includes all patients
write(PatientList, "OriginalData/PatientListFastaFiles.txt")	#only patients with enough samples 
write.csv(patientinfo, "OriginalData/PatientInfo.csv")	
write.csv(AAseqs,"OriginalData/AAseqs.csv")
