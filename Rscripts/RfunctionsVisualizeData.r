...
#Protein=PRO 
	...
	for (Rsite in 1:length(PImuts$pos)){
	if (ListOfSegSites$codon[site]==PImuts$pos[Rsite]){
	if (IsThereAMatch(paste(ListOfSegSites$codon1[site],ListOfSegSites$codon2[site],ListOfSegSites$codon3[site],ListOfSegSites$codon4[site],sep=""),PImuts$mut[Rsite])){ListOfSegSites$resistancecodon[site]<-"PI"
			ListOfSegSites$resistanceaa[site]<-PImuts$mut[Rsite]}}}}		
