...
#  setwd("~/Dropbox/ExplDataAnalysisR/Kadie/ClonalInteference/RScripts")
  source("Rscripts/RfunctionsVisualizeData.r")	
	...
	#read.csv("../SweepOverviewsweeps.csv",row.names=1)->SweepOverview  #data on 30 patients
	library(ape)
	library(seqinr)
	library(pegas)	
...
#open a png file to make a figure
	...
	#	"../Output/Graph2Analyze/Nucleotide/"
...
#indicate the locations of important resistance mutations (codon number and grey color)
		...
		for (p in PatSpecResistancePositions){
			for (d in 1:length(days)){
				...
				rect(which(SitesToDraw==p)-wl-0.1,height[1]-.5,which(SitesToDraw==p)+wr+0.1,height[2]+0.5,density=-1,col="grey",lwd=0, border="grey")}}
				...
				for (cod in respos){	
					for (seq in seqlabels){
						...
						if (gene=="PI") {
							...
							if (length(grep(translate(patfasta[num2,y]),PImuts$mut[x]))>0){co="lightblue"
...
#draw colored rectangles for the 606 site. 
		if (length(which(PatSpecResistancePositions==606))>0){
			s=606
			for (seq in seqlabels){
				num=which(seqlabels==seq)+which(days==substr(seq,5,7))-1
				num2=which(names(patfasta[,1])==seq)
				coltext="black"
				if (patfasta[num2,s]!="a"){
					if (patfasta[num2,s]=="g"){co="black"; coltext="white"}
					if (patfasta[num2,s]=="c"){co="yellow"; coltext="black"}
					if (patfasta[num2,s]=="t"){co="purple"; coltext="black"}
					rect(which(SitesToDraw==s)-HE,num-HE,which(SitesToDraw==s)+HE,num+HE,density=-1,col=co,lwd=0, border=co)}
				text(which(SitesToDraw==s),num,toupper(patfasta[num2,s]),col=coltext,cex=cexpos*1)}
			for (s in c(604,605)){
				for (seq in seqlabels){
					num=which(seqlabels==seq)+which(days==substr(seq,5,7))-1
					num2=which(names(patfasta[,1])==seq)
					if (patfasta[num2,s]!=L$majoritydayzero[s]){
						rect(which(SitesToDraw==s)-HE,num-HE,which(SitesToDraw==s)+HE,num+HE,density=-1,col="purple",lwd=0, border="purple")
						text(which(SitesToDraw==s),num,toupper(patfasta[num2,s]),col="black",cex=cexpos*1)}
					if (patfasta[num2,s]==L$majoritydayzero[s]){text(which(SitesToDraw==s),num,toupper(patfasta[num2,s]),col=1,cex=cexpos*1)}
		...
		for (s in PatSpecResistancePositions){
			if (s<604|s>606){
				for (seq in seqlabels){
					num=which(seqlabels==seq)+which(days==substr(seq,5,7))-1
					num2=which(names(patfasta[,1])==seq)
#					if (patfasta[num2,s]!=L$majoritydayzero[s]){ # compare to majority day 0 for that patient
					if (patfasta[num2,s]!=as.character(consensusB)[s]){ # compare to majority day 0 for that patient
					#co="purple"
					#rect(which(SitesToDraw==s)-HE,num-HE,which(SitesToDraw==s)+HE,num+HE,density=-1,col=co,lwd=0, border=co)
					  rect(which(SitesToDraw==s)-HE,num-HE,which(SitesToDraw==s)+HE,num+HE,density=-1,col="purple",lwd=0, border="purple")
					  text(which(SitesToDraw==s),num,toupper(patfasta[num2,s]),col="black",cex=cexpos*1)}
